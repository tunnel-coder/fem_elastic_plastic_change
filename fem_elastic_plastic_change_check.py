...
class Application(Frame):
    ...
    def solver(self):
        ...
        kxt = np.transpose(kx)
        kzt = np.transpose(kz)
        kx_r = np.reshape(kxt, (n, 1))
        kz_r = np.reshape(kzt, (n, 1))
        ...
        for kst in range(0, nst + 1):
            ...
            for it in range(1, niter + 1):
                ...
                for el in range(1, nel + 1):
                    k = np.int32(elements[el - 1, 0])
                    ki = np.int32(elements[el - 1, 1])
                    kj = np.int32(elements[el - 1, 2])
                    km = np.int32(elements[el - 1, 3])
                    # Stiffness_matrix_element
                    xi, xj, xm = kx_r[ki - 1], kx_r[kj - 1], kx_r[km - 1]
                    zi, zj, zm = kz_r[ki - 1], kz_r[kj - 1], kz_r[km - 1]
